...
;(async () => {
  ...
  await click(page, 'Results', 'button'); await sleep(900)
  const before = await page.evaluate(() => [...document.querySelectorAll('button')].filter(b => b.textContent.includes('Sign off')).length)
  console.log('results awaiting sign-off:', before)
  await shot('p2-m7-results')

  // Sign off the first result
  await clickMaybe(page, 'Sign off', 'button'); await sleep(900)
  const after = await page.evaluate(() => [...document.querySelectorAll('button')].filter(b => b.textContent.includes('Sign off')).length)
  console.log('after sign-off:', after)
  await shot('p2-m7-signed')

  // Alerts tab shows the critical lab value escalation
  await click(page, 'Alerts', 'button'); await sleep(700)
  await shot('p2-m7-alerts')
