...
;(async () => {
  ...
  await click(page, 'AI Assistant', 'a'); await sleep(1600); await shot('ai-greeting')

  // Cohort question via suggestion chip
  await click(page, 'Who has diabetes?', 'button'); await sleep(1200)
  await click(page, 'Which patients are high-priority?', 'button'); await sleep(1200); await shot('ai-cohort')

  // Focus a patient, then ask for a summary
  await page.select('select', 'PT-20394'); await sleep(500)
  await click(page, 'Summarise Kiran', 'button'); await sleep(1200); await shot('ai-patient')
