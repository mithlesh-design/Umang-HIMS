...
async function typeInto(page, sel, text) { await page.evaluate((s, t) => { const el = document.querySelector(s); Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(el, t); el.dispatchEvent(new Event('input', { bubbles: true })) }, sel, text); await sleep(150) }
;(async () => {
  ...
  await clickMaybe(page, 'Sunita Joshi', 'button'); await sleep(700)
  await typeInto(page, 'input[placeholder^="Message"]', marker + ' — please reschedule my 3pm follow-ups')
  await clickMaybe(page, '', 'button[aria-label="Send"]'); await sleep(900)

  await clickAria(page, 'Log out'); await sleep(1200)
  await clickMaybe(page, 'Operations', 'button'); await sleep(500)
  await click(page, 'Reception', 'button'); await sleep(2500)
  await click(page, 'Messaging', 'a'); await sleep(1500)
  await clickMaybe(page, 'Staff messages', 'button'); await sleep(1000)
  ...
  console.log('doctor message visible in RECEPTION portal?', seen)
  await shot('v3-m4-reception')
