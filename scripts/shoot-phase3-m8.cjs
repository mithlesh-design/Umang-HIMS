...
;(async () => {
  ...
  await click(page, 'IPD / Inpatients', 'a'); await sleep(1500)
  await shot('p3-m8-table')
  const flags = await page.evaluate(() => [...document.querySelectorAll('tbody tr')].map(tr => tr.children[5]?.innerText.trim()))
  console.log('AI flags:', JSON.stringify(flags))

  // Quick-peek Sunita (critical)
  await clickMaybe(page, 'Sunita Devi', 'tr'); await sleep(800)
  await shot('p3-m8-peek')
  // Open full chart -> Overview insight + NEWS chip
  await clickMaybe(page, 'Open full chart', 'button'); await sleep(1600)
  await shot('p3-m8-chart')

  // Inbox alerts (deterioration / overdue pushed)
  await click(page, 'Inbox', 'a'); await sleep(1200)
  await clickMaybe(page, 'Alerts', 'button'); await sleep(800)
  await shot('p3-m8-alerts')
