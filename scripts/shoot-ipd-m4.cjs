...
;(async () => {
  ...
  await click(page, 'Open chart', 'button'); await sleep(900)   // Kiran
  await click(page, 'Initiate discharge', 'button'); await sleep(700)
  for (let i = 0; i < 5; i++) { const ok = await clickMaybe(page, 'Clear', 'button'); if (!ok) break; await sleep(300) }
  await click(page, 'Save summary', 'button'); await sleep(500)
  await click(page, 'Complete discharge', 'button'); await sleep(800); await shot('m4-doctor')
  ...
  await click(page, 'IPD / Admission', 'a'); await sleep(1400); await shot('m4-patient')
