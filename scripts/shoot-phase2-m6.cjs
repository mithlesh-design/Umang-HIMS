...
;(async () => {
  ...
  // Open Kiran Patil from the queue, set a diagnosis, complete the consult
  await clickMaybe(page, 'Kiran Patil', 'button'); await sleep(900)
  await typeInto(page, 'input[placeholder^="E.g. Acute Viral"]', 'Stable angina — observation')
  await sleep(300)
  await click(page, 'Complete consultation', 'button'); await sleep(1000)
  await shot('p2-m6-completed')

  // Go to Patient Records, open Kiran, verify the new visit
  await click(page, 'Patient Records', 'a'); await sleep(1400)
  await clickMaybe(page, 'Kiran Patil', 'tr'); await sleep(900)
  await shot('p2-m6-record')
  const hasVisit = await page.evaluate(() => document.body.innerText.includes('Stable angina'))
  console.log('new visit in patient history?', hasVisit)
