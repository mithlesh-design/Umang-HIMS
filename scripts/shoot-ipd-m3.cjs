const puppeteer = require('puppeteer-core')
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe'
const OUT = 'C:\\Users\\Dell\\AppData\\Local\\Temp\\hms-shots'
const BASE = 'http://localhost:3000'
const sleep = (ms) => new Promise((r) => setTimeout(r, ms))
async function click(page, text, sel = 'button, a', tries = 30) {
  for (let i = 0; i < tries; i++) {
    const ok = await page.evaluate((t, sel) => {
      const el = [...document.querySelectorAll(sel)].find((e) => (e.textContent || '').replace(/\s+/g, ' ').trim().includes(t) && !e.disabled)
      if (el) { el.click(); return true }; return false
    }, text, sel)
    if (ok) { await sleep(250); return true }; await sleep(200)
  }
  throw new Error('not found: ' + text)
}
async function clickAria(page, label) { return page.evaluate((l) => { const el = [...document.querySelectorAll('button,a')].find(e => e.getAttribute('aria-label') === l); if (el) { el.click(); return true } return false }, label) }
;(async () => {
  const browser = await puppeteer.launch({ executablePath: CHROME, headless: true, args: ['--no-sandbox'] })
  const page = await browser.newPage()
  await page.setViewport({ width: 1500, height: 1080, deviceScaleFactor: 1 })
  const errors = []
  page.on('console', (m) => { if (m.type() === 'error') errors.push('console: ' + m.text().slice(0, 150)) })
  page.on('pageerror', (e) => errors.push('pageerror: ' + e.message.slice(0, 150)))
  const shot = async (n) => { await sleep(550); await page.screenshot({ path: `${OUT}\\${n}.png` }); console.log('shot', n) }

  await page.goto(`${BASE}/`, { waitUntil: 'networkidle2' }); await sleep(500)
  await click(page, 'Doctor', 'button'); await sleep(2500)

  // Doctor: open Kiran's chart (first inpatient), request surgery
  await click(page, 'IPD / Inpatients', 'a'); await sleep(1400)
  await click(page, 'Open chart', 'button'); await sleep(900)   // Kiran Patil = first card
  await page.type('input[placeholder^="Procedure"]', 'Coronary Angiography'); await sleep(150)
  await page.type('input[placeholder="Surgeon"]', 'Dr. Rohan Mehta'); await sleep(150)
  await page.type('input[placeholder="Indication"]', 'Evaluate coronary anatomy'); await sleep(150)
  await click(page, 'Request surgery', 'button'); await sleep(900); await shot('m3-doctor-surgery')

  // Switch to patient (logout = SPA push, store persists)
  await clickAria(page, 'Log out'); await sleep(1200)
  await click(page, 'Patient', 'button'); await click(page, 'Patient Portal', 'button'); await sleep(2500)
  await page.setViewport({ width: 1366, height: 1700, deviceScaleFactor: 1 })
  await click(page, 'IPD / Admission', 'a'); await sleep(1400); await shot('m3-patient-consent')
  await click(page, 'Give consent for procedure', 'button'); await sleep(900); await shot('m3-patient-consented')

  await browser.close()
  console.log('ERRORS(' + errors.length + '):'); errors.forEach(e => console.log(' ', e)); console.log('DONE')
})().catch((e) => { console.error('ERR', e.message); process.exit(1) })
