/* Final sweep: visit every reception nav item, capture console/runtime errors. */
...
;(async () => {
  ...
  const nav = ['Dashboard', 'OPD Queue', 'OPD Display', 'Appointments', 'Patients',
    'Bed Status', 'Billing Status', 'TPA / Insurance', 'Diagnostics', 'Ambulance',
    'Messaging', 'Download Center', 'Reports', 'Setup']
  for (const item of nav) {
    label = item
    const ok = await click(page, item, 'a')
    if (!ok) errors.push(`[${item}] NAV-FAIL`)
    await sleep(900)
  }
  await browser.close()
  console.log('===== RECEPTION SWEEP — ERRORS (' + errors.length + ') =====')
